...
import 'package:grex_bov/modelo/IdVacina.dart';
...
//terminar manipulação do nome...
  ...
  editarVacina(Vacina v) async {
    ...
    List<IdVacina> vTemp = <IdVacina>[];

    for (var x in idVac) {
      IdVacina vac = IdVacina.deMapParaModel(x);
    ...
    for (var c in ids){
      if (c.nomeVacaId == Dados.vacaVac[0] && c.nomeVacina == Dados.vA){
        _db.alterarVacina(v, c.id_vaca);
        Navigator.push(
            context, MaterialPageRoute(builder: (context) => Partos()));
